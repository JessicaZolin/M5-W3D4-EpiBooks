  ...
  if (filteredBooks.length === 0) {                                                   // if the filteredBooks array is empty, display a warning message
    ...
    return (
      ...
      <Row className="container d-flex justify-content-center flex-wrap m-auto">
        <Col md={9} className='d-flex flex-wrap'>                                     {/* display the books in 3 columns through the SingleBook component (props passed to SingleBook component: book, selected, setSelected)*/}
          {filteredBooks.map((book, index) => {
            return (
              <Col key={index} md={4} className='d-flex justify-content-center flex-wrap mt-4 p-0 pb-4 m-auto'>
